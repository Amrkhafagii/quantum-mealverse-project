-- Phase 1: Critical Database Security Fixes (Final version)
...
SELECT 
  ...
  CASE 
    WHEN MAX(ws.completed_at) IS NOT NULL 
    THEN CURRENT_DATE - MAX(ws.completed_at)::date 
    ELSE NULL 
...
-- 5. Update AuthContext to use rate limiting
CREATE OR REPLACE FUNCTION secure_sign_in(
  p_email TEXT,
  p_password TEXT,
...
BEGIN
  ...
  IF NOT check_auth_rate_limit(p_email, 'login') THEN
    RETURN jsonb_build_object('error', 'Rate limit exceeded. Please try again later.');
  END IF;
  
  -- Log security event
  PERFORM log_security_event(
    NULL,  -- user_id will be null for failed attempts
    'login_attempt',
    jsonb_build_object('email', p_email, 'success', false),
    p_ip_address,
    NULL
  );
