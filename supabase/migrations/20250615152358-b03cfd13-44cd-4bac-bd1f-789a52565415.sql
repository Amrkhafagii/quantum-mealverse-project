...
-- Fix RLS policies for order status updates (corrected version)

-- Drop all existing customer update policies
DROP POLICY IF EXISTS "Customers can update their own orders" ON orders;
DROP POLICY IF EXISTS "Customers can update basic order fields" ON orders;
DROP POLICY IF EXISTS "Customers can cancel their own orders" ON orders;
DROP POLICY IF EXISTS "Allow system status updates on customer orders" ON orders;
