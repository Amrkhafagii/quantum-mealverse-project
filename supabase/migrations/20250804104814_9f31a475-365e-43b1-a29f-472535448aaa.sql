...
INSERT INTO workout_exercises (workout_id, exercise_id, order_index, target_sets, target_reps, target_duration, rest_time_seconds, notes)
SELECT 126, e.id, 
  CASE e.name
  ...
  CASE e.name
    ...
    WHEN 'Bird Dog' THEN 30
    WHEN 'Cat-Cow Stretch' THEN 45
    WHEN 'Child''s Pose' THEN 60
    WHEN 'Hip Circles' THEN 30
  ...
  'Hold for the target duration' as notes
FROM exercises e
...
SELECT 124, e.id,
  ...
  CASE e.name
    WHEN 'Plank' THEN 30
    WHEN 'Wall Sit' THEN 30
    ELSE NULL
  END as target_duration,
  60 as rest_time_seconds,
  'Start with modified versions if needed' as notes
FROM exercises e
...
SELECT 125, e.id,
  ...
  'Keep heart rate elevated' as notes
FROM exercises e
...
SELECT 127, e.id,
  ...
  'Focus on pushing muscles' as notes
FROM exercises e
...
SELECT 128, e.id,
  ...
  NULL as target_duration,
  75 as rest_time_seconds,
  'Focus on pulling muscles' as notes
FROM exercises e
...
SELECT 129, e.id,
  ...
  END as target_reps,
  CASE e.name
    WHEN 'Wall Sit' THEN 45
    ELSE NULL
  END as target_duration,
  90 as rest_time_seconds,
  'Focus on leg strength' as notes
FROM exercises e
...
SELECT 130, e.id,
  ...
  'High intensity, short rest' as notes
FROM exercises e
...
SELECT 131, e.id,
  ...
  CASE e.name
    ...
    WHEN 'Hollow Body Hold' THEN 20
    ELSE NULL
  END as target_duration,
  45 as rest_time_seconds,
  'Focus on core stability' as notes
FROM exercises e
...
SELECT 132, e.id,
  ...
  NULL as target_duration,
  75 as rest_time_seconds,
  'Complete upper body workout' as notes
FROM exercises e
