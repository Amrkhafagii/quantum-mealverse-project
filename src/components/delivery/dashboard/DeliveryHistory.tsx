...
import { getPastDeliveryAssignments } from '@/services/delivery/deliveryAssignmentService';
...
export const DeliveryHistory: React.FC<DeliveryHistoryProps> = ({ deliveryUserId }) => {
  ...
  const loadPastAssignments = async () => {
    try {
      setLoading(true);
      const limit = 5;
      const { assignments, count } = await getPastDeliveryAssignments(deliveryUserId, page, limit);
      ...
      toast({
        ...
        description: "There was a problem loading your delivery history.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  if (pastAssignments.length === 0 && page === 1) {
    return (
