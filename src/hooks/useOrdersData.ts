...
export const useOrdersData = (userId?: string) => { // userId is UUID string from auth
  ...
  const fetchActiveOrders = useCallback(async () => {
    ...
    try {
      // Use separate NOT clauses instead of the problematic "not.in" syntax
      const { data, error } = await supabase
        .from('orders')
        .select('*, order_items(*)')
        .eq('customer_id', userId) // customer_id expects UUID string from auth
